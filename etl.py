...
def process_song_data(spark, input_data, output_data):
    ...
    """
    Reads the song_data JSON file, and creates the songs and artists tables per specs.
    The tables are written into the S3 bucket as parquet files.

    Arguments:

    Spark:      The current spark session object being used.
    input_data: Directory path of where raw song_data exists (S3 bucket).
    output_data:Directory path of where processed data will be stored (S3 bucket).
...
def process_log_data(spark, input_data, output_data):
    ...
    """
    Reads the log_data JSON file, and creates the songs and artists tables per specs.
    The tables are written into the S3 bucket as parquet files.
    The fact table "songplays" is created by joining the songs table with the logs table.

    Arguments:
        spark:      The current spark session being used.
        input_data: Directory path of where raw log_data exists (S3 bucket).
        input_data: Directory path of where processed data will be stored (S3 bucket).
...
def main():
    ...
    The tables are then generated by spark and stored into the S3 buckets.
